/*
gantry

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the CharacterTextSplitterOptions type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CharacterTextSplitterOptions{}

// CharacterTextSplitterOptions struct for CharacterTextSplitterOptions
type CharacterTextSplitterOptions struct {
	Separator *string `json:"separator,omitempty"`
	IsSeparatorRegex *bool `json:"is_separator_regex,omitempty"`
	ChunkSize *int32 `json:"chunk_size,omitempty"`
	ChunkOverlap *int32 `json:"chunk_overlap,omitempty"`
	KeepSeparator *KeepSeparator `json:"keep_separator,omitempty"`
}

// NewCharacterTextSplitterOptions instantiates a new CharacterTextSplitterOptions object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCharacterTextSplitterOptions() *CharacterTextSplitterOptions {
	this := CharacterTextSplitterOptions{}
	return &this
}

// NewCharacterTextSplitterOptionsWithDefaults instantiates a new CharacterTextSplitterOptions object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCharacterTextSplitterOptionsWithDefaults() *CharacterTextSplitterOptions {
	this := CharacterTextSplitterOptions{}
	return &this
}

// GetSeparator returns the Separator field value if set, zero value otherwise.
func (o *CharacterTextSplitterOptions) GetSeparator() string {
	if o == nil || IsNil(o.Separator) {
		var ret string
		return ret
	}
	return *o.Separator
}

// GetSeparatorOk returns a tuple with the Separator field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CharacterTextSplitterOptions) GetSeparatorOk() (*string, bool) {
	if o == nil || IsNil(o.Separator) {
		return nil, false
	}
	return o.Separator, true
}

// HasSeparator returns a boolean if a field has been set.
func (o *CharacterTextSplitterOptions) HasSeparator() bool {
	if o != nil && !IsNil(o.Separator) {
		return true
	}

	return false
}

// SetSeparator gets a reference to the given string and assigns it to the Separator field.
func (o *CharacterTextSplitterOptions) SetSeparator(v string) {
	o.Separator = &v
}

// GetIsSeparatorRegex returns the IsSeparatorRegex field value if set, zero value otherwise.
func (o *CharacterTextSplitterOptions) GetIsSeparatorRegex() bool {
	if o == nil || IsNil(o.IsSeparatorRegex) {
		var ret bool
		return ret
	}
	return *o.IsSeparatorRegex
}

// GetIsSeparatorRegexOk returns a tuple with the IsSeparatorRegex field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CharacterTextSplitterOptions) GetIsSeparatorRegexOk() (*bool, bool) {
	if o == nil || IsNil(o.IsSeparatorRegex) {
		return nil, false
	}
	return o.IsSeparatorRegex, true
}

// HasIsSeparatorRegex returns a boolean if a field has been set.
func (o *CharacterTextSplitterOptions) HasIsSeparatorRegex() bool {
	if o != nil && !IsNil(o.IsSeparatorRegex) {
		return true
	}

	return false
}

// SetIsSeparatorRegex gets a reference to the given bool and assigns it to the IsSeparatorRegex field.
func (o *CharacterTextSplitterOptions) SetIsSeparatorRegex(v bool) {
	o.IsSeparatorRegex = &v
}

// GetChunkSize returns the ChunkSize field value if set, zero value otherwise.
func (o *CharacterTextSplitterOptions) GetChunkSize() int32 {
	if o == nil || IsNil(o.ChunkSize) {
		var ret int32
		return ret
	}
	return *o.ChunkSize
}

// GetChunkSizeOk returns a tuple with the ChunkSize field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CharacterTextSplitterOptions) GetChunkSizeOk() (*int32, bool) {
	if o == nil || IsNil(o.ChunkSize) {
		return nil, false
	}
	return o.ChunkSize, true
}

// HasChunkSize returns a boolean if a field has been set.
func (o *CharacterTextSplitterOptions) HasChunkSize() bool {
	if o != nil && !IsNil(o.ChunkSize) {
		return true
	}

	return false
}

// SetChunkSize gets a reference to the given int32 and assigns it to the ChunkSize field.
func (o *CharacterTextSplitterOptions) SetChunkSize(v int32) {
	o.ChunkSize = &v
}

// GetChunkOverlap returns the ChunkOverlap field value if set, zero value otherwise.
func (o *CharacterTextSplitterOptions) GetChunkOverlap() int32 {
	if o == nil || IsNil(o.ChunkOverlap) {
		var ret int32
		return ret
	}
	return *o.ChunkOverlap
}

// GetChunkOverlapOk returns a tuple with the ChunkOverlap field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CharacterTextSplitterOptions) GetChunkOverlapOk() (*int32, bool) {
	if o == nil || IsNil(o.ChunkOverlap) {
		return nil, false
	}
	return o.ChunkOverlap, true
}

// HasChunkOverlap returns a boolean if a field has been set.
func (o *CharacterTextSplitterOptions) HasChunkOverlap() bool {
	if o != nil && !IsNil(o.ChunkOverlap) {
		return true
	}

	return false
}

// SetChunkOverlap gets a reference to the given int32 and assigns it to the ChunkOverlap field.
func (o *CharacterTextSplitterOptions) SetChunkOverlap(v int32) {
	o.ChunkOverlap = &v
}

// GetKeepSeparator returns the KeepSeparator field value if set, zero value otherwise.
func (o *CharacterTextSplitterOptions) GetKeepSeparator() KeepSeparator {
	if o == nil || IsNil(o.KeepSeparator) {
		var ret KeepSeparator
		return ret
	}
	return *o.KeepSeparator
}

// GetKeepSeparatorOk returns a tuple with the KeepSeparator field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CharacterTextSplitterOptions) GetKeepSeparatorOk() (*KeepSeparator, bool) {
	if o == nil || IsNil(o.KeepSeparator) {
		return nil, false
	}
	return o.KeepSeparator, true
}

// HasKeepSeparator returns a boolean if a field has been set.
func (o *CharacterTextSplitterOptions) HasKeepSeparator() bool {
	if o != nil && !IsNil(o.KeepSeparator) {
		return true
	}

	return false
}

// SetKeepSeparator gets a reference to the given KeepSeparator and assigns it to the KeepSeparator field.
func (o *CharacterTextSplitterOptions) SetKeepSeparator(v KeepSeparator) {
	o.KeepSeparator = &v
}

func (o CharacterTextSplitterOptions) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CharacterTextSplitterOptions) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Separator) {
		toSerialize["separator"] = o.Separator
	}
	if !IsNil(o.IsSeparatorRegex) {
		toSerialize["is_separator_regex"] = o.IsSeparatorRegex
	}
	if !IsNil(o.ChunkSize) {
		toSerialize["chunk_size"] = o.ChunkSize
	}
	if !IsNil(o.ChunkOverlap) {
		toSerialize["chunk_overlap"] = o.ChunkOverlap
	}
	if !IsNil(o.KeepSeparator) {
		toSerialize["keep_separator"] = o.KeepSeparator
	}
	return toSerialize, nil
}

type NullableCharacterTextSplitterOptions struct {
	value *CharacterTextSplitterOptions
	isSet bool
}

func (v NullableCharacterTextSplitterOptions) Get() *CharacterTextSplitterOptions {
	return v.value
}

func (v *NullableCharacterTextSplitterOptions) Set(val *CharacterTextSplitterOptions) {
	v.value = val
	v.isSet = true
}

func (v NullableCharacterTextSplitterOptions) IsSet() bool {
	return v.isSet
}

func (v *NullableCharacterTextSplitterOptions) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCharacterTextSplitterOptions(val *CharacterTextSplitterOptions) *NullableCharacterTextSplitterOptions {
	return &NullableCharacterTextSplitterOptions{value: val, isSet: true}
}

func (v NullableCharacterTextSplitterOptions) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCharacterTextSplitterOptions) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


