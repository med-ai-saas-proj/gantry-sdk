/*
gantry

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"fmt"
	"gopkg.in/validator.v2"
)

// MessagesInner - struct for MessagesInner
type MessagesInner struct {
	ActivityMessage *ActivityMessage
	AssistantMessage *AssistantMessage
	DeveloperMessage *DeveloperMessage
	ReasoningMessage *ReasoningMessage
	SystemMessage *SystemMessage
	ToolMessage *ToolMessage
	UserMessage *UserMessage
}

// ActivityMessageAsMessagesInner is a convenience function that returns ActivityMessage wrapped in MessagesInner
func ActivityMessageAsMessagesInner(v *ActivityMessage) MessagesInner {
	return MessagesInner{
		ActivityMessage: v,
	}
}

// AssistantMessageAsMessagesInner is a convenience function that returns AssistantMessage wrapped in MessagesInner
func AssistantMessageAsMessagesInner(v *AssistantMessage) MessagesInner {
	return MessagesInner{
		AssistantMessage: v,
	}
}

// DeveloperMessageAsMessagesInner is a convenience function that returns DeveloperMessage wrapped in MessagesInner
func DeveloperMessageAsMessagesInner(v *DeveloperMessage) MessagesInner {
	return MessagesInner{
		DeveloperMessage: v,
	}
}

// ReasoningMessageAsMessagesInner is a convenience function that returns ReasoningMessage wrapped in MessagesInner
func ReasoningMessageAsMessagesInner(v *ReasoningMessage) MessagesInner {
	return MessagesInner{
		ReasoningMessage: v,
	}
}

// SystemMessageAsMessagesInner is a convenience function that returns SystemMessage wrapped in MessagesInner
func SystemMessageAsMessagesInner(v *SystemMessage) MessagesInner {
	return MessagesInner{
		SystemMessage: v,
	}
}

// ToolMessageAsMessagesInner is a convenience function that returns ToolMessage wrapped in MessagesInner
func ToolMessageAsMessagesInner(v *ToolMessage) MessagesInner {
	return MessagesInner{
		ToolMessage: v,
	}
}

// UserMessageAsMessagesInner is a convenience function that returns UserMessage wrapped in MessagesInner
func UserMessageAsMessagesInner(v *UserMessage) MessagesInner {
	return MessagesInner{
		UserMessage: v,
	}
}


// Unmarshal JSON data into one of the pointers in the struct
func (dst *MessagesInner) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into ActivityMessage
	err = newStrictDecoder(data).Decode(&dst.ActivityMessage)
	if err == nil {
		jsonActivityMessage, _ := json.Marshal(dst.ActivityMessage)
		if string(jsonActivityMessage) == "{}" { // empty struct
			dst.ActivityMessage = nil
		} else {
			if err = validator.Validate(dst.ActivityMessage); err != nil {
				dst.ActivityMessage = nil
			} else {
				match++
			}
		}
	} else {
		dst.ActivityMessage = nil
	}

	// try to unmarshal data into AssistantMessage
	err = newStrictDecoder(data).Decode(&dst.AssistantMessage)
	if err == nil {
		jsonAssistantMessage, _ := json.Marshal(dst.AssistantMessage)
		if string(jsonAssistantMessage) == "{}" { // empty struct
			dst.AssistantMessage = nil
		} else {
			if err = validator.Validate(dst.AssistantMessage); err != nil {
				dst.AssistantMessage = nil
			} else {
				match++
			}
		}
	} else {
		dst.AssistantMessage = nil
	}

	// try to unmarshal data into DeveloperMessage
	err = newStrictDecoder(data).Decode(&dst.DeveloperMessage)
	if err == nil {
		jsonDeveloperMessage, _ := json.Marshal(dst.DeveloperMessage)
		if string(jsonDeveloperMessage) == "{}" { // empty struct
			dst.DeveloperMessage = nil
		} else {
			if err = validator.Validate(dst.DeveloperMessage); err != nil {
				dst.DeveloperMessage = nil
			} else {
				match++
			}
		}
	} else {
		dst.DeveloperMessage = nil
	}

	// try to unmarshal data into ReasoningMessage
	err = newStrictDecoder(data).Decode(&dst.ReasoningMessage)
	if err == nil {
		jsonReasoningMessage, _ := json.Marshal(dst.ReasoningMessage)
		if string(jsonReasoningMessage) == "{}" { // empty struct
			dst.ReasoningMessage = nil
		} else {
			if err = validator.Validate(dst.ReasoningMessage); err != nil {
				dst.ReasoningMessage = nil
			} else {
				match++
			}
		}
	} else {
		dst.ReasoningMessage = nil
	}

	// try to unmarshal data into SystemMessage
	err = newStrictDecoder(data).Decode(&dst.SystemMessage)
	if err == nil {
		jsonSystemMessage, _ := json.Marshal(dst.SystemMessage)
		if string(jsonSystemMessage) == "{}" { // empty struct
			dst.SystemMessage = nil
		} else {
			if err = validator.Validate(dst.SystemMessage); err != nil {
				dst.SystemMessage = nil
			} else {
				match++
			}
		}
	} else {
		dst.SystemMessage = nil
	}

	// try to unmarshal data into ToolMessage
	err = newStrictDecoder(data).Decode(&dst.ToolMessage)
	if err == nil {
		jsonToolMessage, _ := json.Marshal(dst.ToolMessage)
		if string(jsonToolMessage) == "{}" { // empty struct
			dst.ToolMessage = nil
		} else {
			if err = validator.Validate(dst.ToolMessage); err != nil {
				dst.ToolMessage = nil
			} else {
				match++
			}
		}
	} else {
		dst.ToolMessage = nil
	}

	// try to unmarshal data into UserMessage
	err = newStrictDecoder(data).Decode(&dst.UserMessage)
	if err == nil {
		jsonUserMessage, _ := json.Marshal(dst.UserMessage)
		if string(jsonUserMessage) == "{}" { // empty struct
			dst.UserMessage = nil
		} else {
			if err = validator.Validate(dst.UserMessage); err != nil {
				dst.UserMessage = nil
			} else {
				match++
			}
		}
	} else {
		dst.UserMessage = nil
	}

	if match > 1 { // more than 1 match
		// reset to nil
		dst.ActivityMessage = nil
		dst.AssistantMessage = nil
		dst.DeveloperMessage = nil
		dst.ReasoningMessage = nil
		dst.SystemMessage = nil
		dst.ToolMessage = nil
		dst.UserMessage = nil

		return fmt.Errorf("data matches more than one schema in oneOf(MessagesInner)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(MessagesInner)")
	}
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src MessagesInner) MarshalJSON() ([]byte, error) {
	if src.ActivityMessage != nil {
		return json.Marshal(&src.ActivityMessage)
	}

	if src.AssistantMessage != nil {
		return json.Marshal(&src.AssistantMessage)
	}

	if src.DeveloperMessage != nil {
		return json.Marshal(&src.DeveloperMessage)
	}

	if src.ReasoningMessage != nil {
		return json.Marshal(&src.ReasoningMessage)
	}

	if src.SystemMessage != nil {
		return json.Marshal(&src.SystemMessage)
	}

	if src.ToolMessage != nil {
		return json.Marshal(&src.ToolMessage)
	}

	if src.UserMessage != nil {
		return json.Marshal(&src.UserMessage)
	}

	return nil, nil // no data in oneOf schemas
}

// Get the actual instance
func (obj *MessagesInner) GetActualInstance() (interface{}) {
	if obj == nil {
		return nil
	}
	if obj.ActivityMessage != nil {
		return obj.ActivityMessage
	}

	if obj.AssistantMessage != nil {
		return obj.AssistantMessage
	}

	if obj.DeveloperMessage != nil {
		return obj.DeveloperMessage
	}

	if obj.ReasoningMessage != nil {
		return obj.ReasoningMessage
	}

	if obj.SystemMessage != nil {
		return obj.SystemMessage
	}

	if obj.ToolMessage != nil {
		return obj.ToolMessage
	}

	if obj.UserMessage != nil {
		return obj.UserMessage
	}

	// all schemas are nil
	return nil
}

// Get the actual instance value
func (obj MessagesInner) GetActualInstanceValue() (interface{}) {
	if obj.ActivityMessage != nil {
		return *obj.ActivityMessage
	}

	if obj.AssistantMessage != nil {
		return *obj.AssistantMessage
	}

	if obj.DeveloperMessage != nil {
		return *obj.DeveloperMessage
	}

	if obj.ReasoningMessage != nil {
		return *obj.ReasoningMessage
	}

	if obj.SystemMessage != nil {
		return *obj.SystemMessage
	}

	if obj.ToolMessage != nil {
		return *obj.ToolMessage
	}

	if obj.UserMessage != nil {
		return *obj.UserMessage
	}

	// all schemas are nil
	return nil
}

type NullableMessagesInner struct {
	value *MessagesInner
	isSet bool
}

func (v NullableMessagesInner) Get() *MessagesInner {
	return v.value
}

func (v *NullableMessagesInner) Set(val *MessagesInner) {
	v.value = val
	v.isSet = true
}

func (v NullableMessagesInner) IsSet() bool {
	return v.isSet
}

func (v *NullableMessagesInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMessagesInner(val *MessagesInner) *NullableMessagesInner {
	return &NullableMessagesInner{value: val, isSet: true}
}

func (v NullableMessagesInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMessagesInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


